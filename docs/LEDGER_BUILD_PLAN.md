...
- WP-4 Connector kit and simulators (2.5 hours): interface, registry, hris, idp, mdm, shipping, files simulators with effective dating; Workday and Okta stubs; Google Docs connector real. Acceptance: `hris.get_worker("priya", asOf = Sept 10)` returns the old address and `asOf = today` the new one; the doc is created and sections written.
- WP-5 Sheet UI (3 hours): Work tab, Workers tab, evidence drawer, time slider, refusal on typing done, runs page with export. Acceptance: the slider replays; typing done is refused with the reason; revoke works from the Workers tab.
- WP-6 Slack surface (2 hours): manifest, events, slash command, approval buttons, per agent posting, thread per run. Acceptance: `/ledger new "Priya starts Monday as a sales engineer in Austin"` creates the run; Dan's button press resolves the approval; a revoked agent posts nothing.
- WP-7 Demo runner and audit export (1.5 hours): `make demo`, `scripts/export_audit.ts`. Acceptance: the scenario runs end to end unattended in under four minutes; the audit page names Dan as the access approver.
...
- WP-10 Sheet data model (2 hours): sheets, columns, cells, proposals; cell provenance; the formula evaluator with `ALL_VERIFIED`, `AS_OF`, `SUM`, `IF`; plan and batch shapes. Acceptance: the Day One run materializes as a plan sheet; a fixture batch sheet with an `agent_step` column runs the agent once per row; formulas recompute on cell change; the time slider replays cell values.
